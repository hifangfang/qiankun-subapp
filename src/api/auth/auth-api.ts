...
export class AuthApi {
  ...
  private static loginApi = MICRO_CONFIG.auth + "/auth/oauth/login";
  ...
  private static zzdLoginApi = MICRO_CONFIG.auth + "/auth/auu/authCode?type=token&authCode=";
  ...
  private static refreshTokenUrl = MICRO_CONFIG.auth + "auth/oauth/token?refreshToken={token}";
  ...
  //删除redis中的token
  private static removeTokenUrl = MICRO_CONFIG.auth + "auth/removeToken";
...
//  获取短信验证码
  private static sendMobileCodeUrl = MICRO_CONFIG.base + "base/shortmsg/sendMobileCode"

  static async sendMobileCode(vo: any): Promise<any> {
    const response = await axios.post<ApiResponse<any>>(this.sendMobileCodeUrl, vo);
    return response.data.data;
  }

// 手机号+短信验证码登录
  private static phonecodeUrl = MICRO_CONFIG.auth + "/auth/auu/phonecode"
