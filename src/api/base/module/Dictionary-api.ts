...
export class DictionaryApi {
  ...
  //异步加载树节点搜素数据新接口
  private static getDictionarySearchTreeUrl = DictionaryApi.urlPrefix+ "base/dictionary/sync-fieldGroup-tree/{subGuid}/{key}";

  static async getDictionarySearchTree(subGuid:any,key:string):Promise<DictionarySearchTreeVO[]>{
    const response =await axios.get<ApiResponse<DictionarySearchTreeVO[]>>(this.getDictionarySearchTreeUrl.replace('{subGuid}',subGuid).replace('{key}',key));
    return response.data.data;
  }
