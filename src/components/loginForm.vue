<template>
    <div  class="login-box">
      <!--启用验证码登录      -->
      <el-tabs v-if="showCodeLogin&&loginType!='authentication'" v-model="activeName" class="login-type-wrap" @tab-click="handleClick">
...
import { WaterMarkService } from '@/utils/js/water-mark.service';
import {Prop} from "vue-property-decorator";
import bus from "@/utils/js/bus";
import ColumnCommon from '@/utils/js/common';
import {WebSoketAPI} from "@/api/websoket/websoket-api";
import {getUrlParam} from "@/utils/js/url_path_utils";
import {LocalStorageUtil} from "@/utils/js/localstorage_utils";
import dateTime from "@/utils/js/dateTime"
@Component({})
export default class LoginComponent extends Vue {
  @Prop() tokenFail: any; // 是否是token失效弹窗
  @Prop() subGuid:any;
  @Prop() beforeUserId:any;
  @Prop() loginType: any; // 登录类型:tokenFail-登录失效弹窗；authentication-二次验证获取脱敏数据弹窗
  loading = false;
  account = {
    username: "",
    pwd: ""
  ...
  created() {
    ...
    if (this.getLoginFormShow()){
      if(this.subGuid){
        ...
        this.account.username=decodeURIComponent((!this.isEmpty(userInfo))?userInfo.user_name:localStorage.user_name);
  ...
  clickEvt() {
    if (this.$refs.password) {
      (this.$refs.password as any).$refs.input.onmousedown = (evt) => {
        if (evt) {
          evt.preventDefault();
          evt.stopPropagation();
        }
        if (this.account.pwd === '' || this.readonly) {
          (this.$refs.password as any).$refs.input.blur();
          setTimeout(() => {
  ...
  async handleLogin() {
    ...
    if(this.activeName=="password"||!this.showCodeLogin){
      (this.$refs.AccountForm as any).validate(async (valid) => {
        if (valid) {
          let result = {
      ...
      (this.$refs.codeForm as any).validate(async (valid) => {
        if (valid) {
          let result = {
...
//  获取短信验证码
  async getMobileCode(){
    ...
    (this.$refs.codeForm as any).validateField(['mobile'], async(Error) => {
      if (!Error) {
        const param={
          subGuid: "",
          telephone:this.codeLogin.mobile
        }
        const res = await AuthApi.sendMobileCode(param);
        this.openTimer()
      } else {
        console.log(Error)
      }
  ...
  initSoket() {
    const loginInfo = {username: localStorage.getItem('username'),
      logintype: 'PC',
      key :'mac',
  ...
  addWaterMarkService(){
    ...
    if(userNickname && waterMark!='' && waterMark != 'true' && waterMark != null&&isIframe!='link'){
      ...
      let currentTime = dateTime.getCurrTime();
      let reg = /[\d]{4}[\/-]{1}[\d]{1,2}[\/-]{1}[\d]{1,2}\s[\d]{1,2}[:][\d]{1,2}[:][\d]{1,2}/g;
      waterMark=JSON.parse(JSON.stringify(waterMark).replace(reg,currentTime))
      setTimeout(()=>{
...
<style lang="scss">
.loginFormDialog,.biLoginFormDialog{
  //锁屏遮罩层添加毛玻璃效果
  &::after{
    content: "";
    position: fixed;
...
.loginFormDialog,.biLoginFormDialog,.iframeLogin{
  .login-box{
    padding:50px;
    width: 470px;
    height: 400px;
